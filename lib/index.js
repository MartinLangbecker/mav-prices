...
const queryPrices = (start, dest, date, opt) => {
  ...
  const travelDate = date.toISOString() || new Date().toISOString();

  // TODO if parameter opts.duration is set, make additional requests for every time window exceeding 8 hours and unify responses
  ...
  return fetch(url, {
    ...
    .then((body) => {
      if (body.route === []) {
        console.warn('Request returned no results');
        return;
      }

      const journeys = [];
      for (const journey in body.route) {
        journeys.push(parseJourney(journey));
      }
