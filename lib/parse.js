...
const getLine = (description) => {
  ...
  for (const segment of segments) {
    ...
    const isMatchingMode = isBus
      ...
      : /\bmetro\b/i.test(segment) || /^M\d/.test(segment) || segment === segments[0];
    if (!isMatchingMode) continue;

    // "line M4", "lines 5", "Line M4"
    const lineMatches = segment.match(/\blines?\s+(\w+)/gi);
    if (lineMatches) lines.push(...lineMatches.map((m) => m.replace(/^lines?\s+/i, '')));
...
const createLeg = (routeSegment) => {
  ...
  if (!routeSegment.trainDetails?.trainNumber && routeSegment.description)
    return { ...createBaseLeg(routeSegment), mode: 'walking', description: routeSegment.description };
...
export const parseJourney = (journey) => {
  if (!journey?.details?.routes) {
    throw new Error(`Invalid journey: missing details.routes (id: ${journey?.sameOfferId})`);
  }
  const travelClass = journey.travelClasses?.[0];
  if (!travelClass?.price) {
    throw new Error(`Invalid journey: missing price data (id: ${journey?.sameOfferId})`);
  }
  return {
    type: 'journey',
