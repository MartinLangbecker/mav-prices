...
const sendRequest = (body, sessionId) =>
  ...
  }).then((data) => data.route.map(parseJourney));
...
export const queryPrices = (from, to, date, opt) => {
  ...
  if (!options.duration) return sendRequest(body, sessionId);
  ...
  for (let i = 0; i < requestCount; i++) {
    body.travelStartDate = addMinutes(travelDate, EIGHT_HOURS * i).toISOString();
    requests.push(sendRequest({ ...body }, sessionId));
  }
